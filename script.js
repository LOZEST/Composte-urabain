...
function updateSimDepositPreview() {
  ...
  const kg = Number(kgInput.value || 2.5);
...
if (simPoints) {
  setInterval(() => {
    ...
    simState.cuve1 = Math.max(12, Math.min(100, simState.cuve1 + Math.round((Math.random() - 0.45) * 3)));
    simState.cuve4 = Math.max(8, Math.min(100, simState.cuve4 + Math.round((Math.random() - 0.5) * 2)));
    renderSim();
  }, 2200);
  renderSim();
}
